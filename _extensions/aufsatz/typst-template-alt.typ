...
#let aufsatz(
  ...
  page(
    ...
    #grid(
      ...
      [Name:], [#if author-text != none { author-text }],
      [Matrikelnummer:], [#if student-id != none { student-id }],
      [Studiengang:], [#if degree-program != none { degree-program }],
      [Fachsemester:], [#if semester != none { semester }],
      [Modul:], [#if module != none { module }],
      [Lehrveranstaltung:], [#if course != none { course }],
      [Dozent/in:], [#if supervisor != none { supervisor }],
      [Hochschule:], [#if institution != none { institution }],
      [Abgabedatum:], [
        #if date != none {
          date
        } else {
